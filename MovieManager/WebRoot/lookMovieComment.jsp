...
<html>
  ...
  <body>
  		<s:form action="findComment" method="post">
	        <s:textfield name="comment" label="输入你要查询评论的关键字" maxLength="12"/>
       ...
       <table id="table" align="center" width="50%" border="1px" >
		...
		<tr>
     		...
     		<td align="center" >
    				<a href="lookMovieComment.action?moviename=<s:property value="moviename"/>" 
    				class="easyui-linkbutton" data-options="iconCls:'icon-search'">查看评论</a>
    		</td>
    	</tr>
   		</s:iterator>
    	</table>
    	 <div class="pagination">
       			<s:if test="#session.moviePage.pageNum-1 >= 1">
					<a href="lookMovieC.action?pageNum=${session.moviePage.pageNum-1 }" class="page"><span>上一页</span></a>
				</s:if>
				<s:else>
					<a class="page active"><span>上一页</span></a>
				</s:else>
      			<s:iterator begin="1" end="#session.moviePage.pageNum-1" var="i">
      					<a href="lookMovieC.action?pageNum=${i}" class="page"><span>${i}</span></a>
      			</s:iterator>
      			...
      			<s:iterator begin="#session.moviePage.pageNum+1" end="#session.totalPage" var="i">
					...
					<a href="lookMovieC.action?pageNum=${session.moviePage.pageNum+1 }" class="page"><span>下一页</span></a>
				</s:if>
				<s:else>
